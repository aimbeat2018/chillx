...
import ott.chillx.network.apis.ConfigurationApi;
import ott.chillx.network.apis.SubscriptionApi;
import ott.chillx.network.model.ActiveStatus;
import ott.chillx.network.model.config.ApkUpdateInfo;
    ...
    private void checkForUpdate(int latestAppVersion) {
        ...
        if (latestAppVersion > version) {
            ...
            if (PreferenceUtils.getUserId(SplashScreenActivity.this) != null) {
                ...
                Uri video = Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.chillx_intro);
                myvideoview.setVideoURI(video);
